...
func LimitAccess(key string, duration time.Duration, times int) (bool, error) {
	...
	if err != nil {
		logger.Logger.Error("判断受限资源key是否存在时错误", err, 0, key)
		return false, err
	}
	...
	if val == 0 { // no exists
		...
		if err != nil {
			logger.Logger.Error("计数是出错", err, 0, key)
			return false, err
		}
	...
	if err != nil {
		logger.Logger.Error("获取key具体数据时出错", err, 0, key)
		return false, err
	}
	...
	//否则就是评率过快
	logger.Logger.Warn("资源访问频率过快", 0, key)
	return false, nil
}
